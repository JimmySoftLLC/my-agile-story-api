...
function containsBlackListedStuff(myString, validationResult) {
  ...
  for (let i = 0; i < notAllowed.length; i++) {
    if (validator.contains(myString, notAllowed[i])) {
      validationResult.offending = notAllowed[i];
      return true;
    }
  }
  return false;
...
module.exports = function validateInputs(req) {
  ...
  if (req.body.oldPassword) {
    if (containsBlackListedStuff(req.body.oldPassword, validationResult)) {
      validationResult.pass = false;
      validationResult.message = 'This is not a valid password.';
    }
  }

  if (containsBlackListedStuff(req.body.firstName, validationResult)) {
    validationResult.pass = false;
    validationResult.message = 'This is not a valid first name.';
  }

  if (containsBlackListedStuff(req.body.lastName, validationResult)) {
    validationResult.pass = false;
    validationResult.message = 'This is not a valid last name.';
  }

  if (containsBlackListedStuff(req.body.bio, validationResult)) {
    validationResult.pass = false;
    validationResult.message = 'This is not a valid bio.';
  }

  if (containsBlackListedStuff(req.body.role, validationResult)) {
    validationResult.pass = false;
    validationResult.message = 'This is not valid role.';
  }

  if (!validationResult.pass) {
    if (validationResult.offending !== '') {
      validationResult.message +=
        '  The word, ' + validationResult.offending + ', is not allowed.';
    } else {
      validationResult.message +=
